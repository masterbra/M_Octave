...
%sum=sum+xbfin;
 ...
 area=((b-a1)*sum)/(3*n);
 fprintf('Valor aprox (va): %12.12f\n', area);
 fprintf('Error verdadero (ER=vv-va): %12.12f\n', vv-area);
 fprintf('Error relativo porcentual%%: %2.2f\n', abs((vv-area)/vv)*100);
  #Calculo del error de Truncamiento 
err=(-1)* ((b-a1)^5/(180*n^4))*(int4/(b-a1));
 fprintf('Error Estimado/tolerable (simpson 13): %5.7f\n', abs(err))
